...
from utils.clip_augmentations import ClipRandAugment
...
if __name__ == '__main__':
    ...
    if dist.get_rank() == 0:
        path = os.path.join(opt.output_dir, "train_3d.config.json")
        with open(path, 'w') as f:
            json.dump(vars(opt), f, indent=2)
        logger.info("Full config saved to {}".format(path))

    logger.info('ra_n is {}, ra_m is {}'.format(opt.ra_n, opt.ra_m))
