...
class VideoDataset(torch.utils.data.Dataset):
    ...
    def _parse_rgb_lmdb(self, video_path, offsets, dup=1):
        ...
        with lmdb_env.begin() as lmdb_txn:
            ...
            for offset in offsets:
                for frame_id in range(offset + 1, offset + self.num_steps * self.clip_length + 1, self.num_steps):
                    frame_id = math.ceil(frame_id / dup)
...
class VideoTrainDataset(VideoDataset):
    def _parse_sample_str(self, sample, video_idx):
        ...
        if duration < self.length_ext:
            dup = math.ceil(self.length_ext / duration)
            duration *= dup
        else:
            dup = 1
        ...
        return video_path, offsets, label, dup
...
class VideoRGBTrainDataset(VideoTrainDataset):
    def __getitem__(self, item):
        video_path, offsets, label, dup = self._parse_sample_str(self.samples[item], item)

        image_list = self._parse_rgb_lmdb(video_path, offsets, dup)
...
class VideoRGBTestDataset(VideoTestDataset):
    def __getitem__(self, item):
        ...
        video_path, offsets, label, dup = self._parse_sample_str(self.samples[item_out], item_out, item_in)
